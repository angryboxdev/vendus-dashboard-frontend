...
function ClassifyPanel({
  ...
  async function handleSave() {
    ...
    try {
      ...
      setError(
        msg.includes("Canal")
          ? "Canal obrigatório para esta subcategoria."
          : msg,
      );
    } finally {
      setSaving(false);
    ...
    <div className="space-y-3 rounded-lg border border-[#F5C992]/40 bg-[#FDF8F5] p-3 text-sm">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div>
          ...
          {selectedCategory && (
            <span
              className={`mt-1 inline-block rounded-full px-2 py-0.5 text-xs ${FINANCIAL_TYPE_COLORS[selectedCategory.financialType as FinancialType]}`}
            >
              {
                FINANCIAL_TYPE_LABELS[
                  selectedCategory.financialType as FinancialType
                ]
              }
            </span>
          )}
